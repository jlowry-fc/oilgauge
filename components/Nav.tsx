...
export default function Nav({ userName, isSuperAdmin }: { userName: string; isSuperAdmin?: boolean }) {
  ...
  const allLinks = isSuperAdmin ? [...links, { href: '/admin', label: '⚙ Admin' }] : links

  return (
    <div style={{ background:'var(--bg2)', borderBottom:'1px solid var(--border)', position:'sticky', top:0, zIndex:100 }}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'12px 20px' }}>
        <span className="serif" style={{ fontSize:'20px', color:'var(--accent)' }}>
          OilGauge <span style={{ color:'var(--text2)', fontFamily:'DM Sans', fontSize:'12px', fontWeight:500 }}>Pro</span>
        </span>
        <div style={{ display:'flex', alignItems:'center', gap:'10px' }}>
          <span style={{ fontSize:'13px', color:'var(--text2)' }}>{userName}</span>
